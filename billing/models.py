...
class Transaction(models.Model):
    ...
    # Здесь auto_now оставлен намеренно, в отличие от created_at выше, и
    # асимметрия осознанная. Смысл колонки — «когда строку меняли в последний
    # раз», и вычислять его должна сама система, иначе значение перестаёт быть
    # достоверным при первой же забытой строчке в коде. Обработка транзакций на
    # это и опирается: она перечисляет updated_at в update_fields, чтобы Django
    # вызвал pre_save. Цена известна и ровно та же, что была у created_at: при
    # переносе данных из другой системы исходное время изменения сохранить
    # нельзя, оно будет перезаписано временем импорта.
