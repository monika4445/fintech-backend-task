...
class ExtendedJSONEncoder(json.JSONEncoder):
    ...
    Граница, о которой стоит знать: `json.dumps` вызывает `default()` только для
    ЗНАЧЕНИЙ. Ключи он обрабатывает отдельной веткой и до `default()` не
    доводит, поэтому `{Decimal("1.5"): "x"}` или `{uuid4(): "x"}` упадут с
    `keys must be str, int, float, bool or None`. Это соответствует самому
    формату: в JSON ключ объекта — всегда строка, так что словарь с ключом
    `UUID` не является JSON-совместимым и отказ здесь правильный. Если такой
    словарь всё же приходит, приводить ключи должен вызывающий, осознанно
    выбрав представление.
